...
export default function BadanGiziLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (maintenanceMode) {
      setError('Sistem sedang dalam mode perawatan. Silakan coba lagi nanti.');
      return;
    }
    ...
    try {
      ...
      if (admin.role !== 'badan_gizi') {
        setError('Akses ditolak. Hanya Badan Gizi yang dapat masuk di halaman ini.');
        setIsVerified(false);
        setIsLoading(false);
        return;
      }

      cookie.setWithMinutes('badan_gizi', admin, sessionTimeoutMinutes);
      navigate('/badan-gizi/dashboard');
    } catch (err) {
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white rounded-2xl overflow-hidden shadow-xl">
          <div className="bg-gradient-to-r from-sky-600 to-sky-700 p-6 text-center">
            ...
            <h2 className="text-xl font-bold text-white">Badan Gizi</h2>
            <p className="text-sky-100 text-sm mt-1">Kelola Data SPPG</p>
          </div>

          <div className="p-6">
            {maintenanceMode && (
              <div className="mb-4 rounded-xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50 px-4 py-3 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-red-900 leading-relaxed">Login untuk admin non-super sementara dinonaktifkan.</p>
                  </div>
                </div>
              </div>
            )}
            {announcement && (
              <div className="mb-4 rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50 px-4 py-3 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-amber-700">Pengumuman Sistem</p>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg outline-none focus:border-sky-500 transition-all text-sm"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-12 py-2.5 border border-slate-300 rounded-lg outline-none focus:border-sky-500 transition-all text-sm"
                  ...
                  <button
                    ...
                    className="absolute inset-y-0 right-0 pr-3 flex items-center text-sky-500 hover:text-sky-700"
                  >
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-sky-600 hover:bg-sky-700 text-white font-bold py-3 rounded-xl transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
          ...
          <div className="px-6 py-4 bg-slate-50 border-t border-slate-100">
            <button
              onClick={() => navigate('/')}
              className="w-full flex items-center justify-center gap-2 text-sky-700 hover:text-sky-900 text-sm font-semibold"
            >
